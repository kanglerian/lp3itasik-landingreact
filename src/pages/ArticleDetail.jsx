...
const ArticleDetail = () => {
  ...
  const listArticles = articles.map((article, i) =>
    <li key={i} data-aos-delay={i * 100}>
      <a href={`/article/` + article.uuid} role="button" className="bg-gray-100 py-1 px-2 rounded text-base text-sky-600 underline md:hover:text-sky-700">{article.title.slice(0, 40) + "..."}</a>
    </li>
  )
                ...
                <div className="flex flex-col justify-end md:flex-row gap-5">
                  <div className="w-full md:w-1/3 order-2 md:order-none">
                    <h1 className="text-xl font-bold text-gray-800">
                      {currentLanguage == 'en' ? 'More articles' : 'Artikel lainnya'}
                    </h1>
                    <hr className="my-2" />
                    <div>
